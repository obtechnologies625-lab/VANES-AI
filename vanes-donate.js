...
function open(){
 ...
 if(!modal){
  ...
  modal.querySelector('#vanesCopyDonate').addEventListener('click',async function(){try{await navigator.clipboard.writeText(NUMBER);this.textContent='Copied ✓';setTimeout(()=>this.textContent='Copy number →',1800)}catch(_){this.textContent=NUMBER}});
 }
 modal.hidden=false;
}
