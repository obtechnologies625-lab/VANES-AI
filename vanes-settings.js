...
function wire(){
 ...
 const feedbackSubmit=async e=>{e?.preventDefault();const status=document.getElementById('feedbackStatus'),name=value('feedbackName'),comment=value('feedbackText'),email=value('feedbackEmail');if(!name){if(status)status.textContent='Please enter your name.';return}if(!selectedRating&&!comment){if(status)status.textContent='Choose a rating or enter a comment first.';return}if(selectedRating)return send('rating',{rating:selectedRating,name,comment,email},status);return send('feedback',{name,comment,email},status)};
